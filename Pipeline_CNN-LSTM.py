...
from keras.layers import Dense, LSTM, Dropout, LayerNormalization, Input, Conv1D, MaxPooling1D
...
def build_model(depth, units, dropout_rate, batch_size, memory, n_features, l1, l2, lr, kernel_size, pool_size):   
    model = Sequential() 

    model.add(Input(batch_shape=(batch_size, memory, n_features)))

    model.add(Conv1D(batch_size, kernel_size, padding='same', activation='relu'))
    model.add(MaxPooling1D(pool_size, padding='same'))
    model.add(LayerNormalization(rms_scaling=True)) 
    
    for i in range(depth):
        model.add(LSTM(units=int(units/(2**i)), stateful=True, kernel_regularizer=None if i>0 else regularizers.L1L2(l1=l1, l2=l2), recurrent_dropout=dropout_rate, return_sequences=True if i<depth-1 else False))
        model.add(LayerNormalization(rms_scaling=True)) 
    ...
    for days_of_memory in n_days_to_analyze:
        ...
        for scenario, desired_columns in scenarios.items():
            ...
            param_grid={'instance__model__units':[32,64,128],'instance__model__dropout_rate':[0, 0.2, 0.4], 
                        ...
                        'instance__model__kernel_size':[3,7,11], 'instance__model__pool_size':[2,5,8]
                       } 
            ...
            #save figure
            plt.savefig('images/plots/CNN-LSTM/{}_{}_{}daysmemory.eps'.format(river, scenario, days_of_memory), format='eps',dpi=100)
            ...
            new_row = {'river':river,'scenario':scenario, 'model':'CNN-LSTM','stateful':'True', 'split':'TimeSeriesSplit', 'folds':folds, 'trials':trials,
                       ...
                       'alpha': None, 'batch_size':batch_size, 'epochs':epochs, 'early_stopping': patience, 'depth':bp['instance__model__depth'],'units':bp['instance__model__units'], 
                        ...
                        'kernel_size':bp['instance__model__kernel_size'], 'pool_size':bp['instance__model__pool_size'], 'activation':'relu', 'padding':'same', 
                        'training_time': round(mean_fit_time,3), 'search_time':round(search_end_time-search_start_time,3)
                        }
